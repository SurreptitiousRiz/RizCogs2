...
class XORole(BaseCog):
    ...
    def save(self):
        try:
            with open(JSON, 'w') as outputfile:
                outputfile.write(json.dump(self.settings))
        except:
    ...
    async def xorole_list(self, ctx, *, roleset: str = None):
        ...
        try:
            ...
            if len(lines) == 1:
                await ctx.send('No roles are available to assign.')
                return
    ...
    async def xorole_add(self, ctx, *, role: str):
        ...
        try:
            ...
            if role in member.roles and len(existing) == 1:
                m = await ctx.send('You already have that role; nothing to do.')
                messages.add(m)
                return
            ...
            m = await ctx.send("Role in roleset %s switched to %s." % (roleset, role.name))
            messages.add(m)

        except XORoleException as e:
            m = await ctx.send(warning(*e.args))
            messages.add(m)
        finally:
        ...
        try:
            ...
            if role:
                if role not in member.roles:
                    m = await ctx.send("You don't have that role; nothing to do.")
                    messages.add(m)
                    return
            ...
            if to_remove:
                ...
                m = await ctx.send('Removed the %s: %s.' % (plural, rlist))
            else:
                m = await ctx.send("You don't belong to any roles in the %s roleset." % role_or_roleset)
    ...
    async def xorole_toggle(self, ctx, *, role_or_roleset: str):
        ...
        try:
            ...
            if not 0 < len(roles) <= 2:
                m = await ctx.send(warning("Cannot toggle within the '%s' roleset." % roleset))
                messages.add(m)
                return
            ...
            if len(roles) == 1:
                ...
                if len(to_remove) != 1:
                    m = await ctx.send(warning("You must have one role in %s to toggle it." % roleset))
                    messages.add(m)
                    return
            ...
            if to_add and to_remove:
                m = await ctx.send('Toggled from %s to %s.' % (to_remove[0], to_add[0]))
            elif to_add:
                m = await ctx.send("Role '%s' added." % to_add[0])
            elif to_remove:
                m = await ctx.send("Role '%s' removed." % to_remove[0])
    ...
    async def xoroleset_addroleset(self, ctx, *, name: str):
        ...
        try:
            if len(name.split()) > 1:
                await ctx.send('For usability reasons, whitespace is not permitted in roleset names. Try again.')
                return
            elif name.lower() in ("all", "server"):
                await ctx.send('The name "%s" is reserved; please pick something else.' % name)
                return

            self.add_roleset(server, name)
            await ctx.send("Roleset '%s' created." % name)
        except XORoleException as e:
            await ctx.send(warning(*e.args))
    ...
    async def xoroleset_rmroleset(self, ctx, *, name: str):
        ...
        try:
            ...
            await ctx.send("Roleset '%s' removed." % name)
        except XORoleException as e:
            await ctx.send(warning(*e.args))
    ...
    async def xoroleset_renroleset(self, ctx, oldname: str, newname: str):
        ...
        try:
            if len(newname.split()) > 1:
                ...
                await ctx.send('The name "%s" is reserved; please pick something else.' % newname)
                return
            ...
            self.update_rolesets(server, rolesets)
            await ctx.send("Rename successful.")
        except XORoleException as e:
            await ctx.send(warning(*e.args))
    ...
    async def xoroleset_audit(self, ctx):
        ...
        try:
            ...
            if not lines:
                await ctx.send('All roleset memberships are singular.')
                return
    ...
    async def xoroleset_autodelete(self, ctx, delay: int = None):
        ...
        if delay is None:
            ...
            if delay:
                return await ctx.send("Auto-delete delay is currently %is." % delay)
            else:
                return await ctx.send("Auto-delete is currently disabled.")
        ...
        if delay:
            return await ctx.send("Auto-delete delay set to %is." % delay)
        else:
            return await ctx.send("Auto-delete disabled.")
    ...
    async def xoroleset_autoswitch(self, ctx, on_off: str = None, *rolesets: str):
        ...
        if on_off and not rolesets:  # missing parameters
            await ctx.send("No rolesets specified.")
    ...
    async def on_member_update(self, before, after):
        ...
        print(self.settings)
        if before.roles != after.roles and before.guild.id in self.settings:
